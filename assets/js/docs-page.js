;(async function initDocsPage() {
  ...
  function renderHero(hero) {
    ...
    const copy = document.createElement('button');
    copy.className = 'docs-copy-link';
    copy.type = 'button';
    copy.setAttribute('aria-label', window.MKSSiteI18n?.get('copy.link', 'Copy link'));
    copy.innerHTML = '<i class="hgi-stroke hgi-link-01" aria-hidden="true"></i>';
    ...
    header.append(scene, eyebrow, meta, title, description, pills, terminal, copy);
    return header;
  }
  ...
  function renderTabbedSection(section) {
    ...
    title.innerHTML = `
      ...
      <div class="docs-section-actions">
        ...
        <button class="docs-copy-link" type="button" aria-label="${escapeHtml(window.MKSSiteI18n?.get('copy.link', 'Copy link'))}">
