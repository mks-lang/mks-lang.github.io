...
function renderFilters(releases) {
  ...
  const searchLabel = escapeHtml(searchPlaceholder);
  const placeholderWithHint = escapeHtml(searchPlaceholder + ' (/)');

  return `
    <section class="section change-filter-shell">
      <div class="change-filters panel glass" data-change-filters>
        <input class="change-search" type="search" placeholder="${placeholderWithHint}" aria-label="${searchLabel}" data-change-search>
